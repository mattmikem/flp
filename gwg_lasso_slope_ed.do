...
quietly sum if year == `y' & group == `j' & bach_plus == `ed'
...
quietly lassoShooting lwage $x_lasso if emarry == 0 & nchild == 0 & bach_plus == `ed', lasiter(100) verbose(0) fdisplay(0)
local ysel `r(selected)'
quietly lassoShooting female $x_lasso if emarry == 0 & nchild == 0 & bach_plus == `ed', lasiter(100) verbose(0) fdisplay(0)
local fsel `r(selected)'
quietly lassoShooting exp $x_lasso if emarry == 0 & nchild == 0 & bach_plus == `ed', lasiter(100) verbose(0) fdisplay(0)
local esel `r(selected)'

local ssel  : list fsel | ysel 
local sesel : list esel | ssel

reg lwage female exp exp2 `sesel' if emarry == 0 & nchild == 0 & bach_plus == `ed', r

mat res_`y'[`j', 1]   = _b[female]
capture mat res_`y'[`j', 2] = _b[exp]
capture mat res_`y'[`j', 3] = _b[exp2]  

quietly lassoShooting lwage $x_lasso if emarry == 1 & female == 1 & bach_plus == `ed', lasiter(100) verbose(0) fdisplay(0)
local ysel `r(selected)'
quietly lassoShooting exp   $x_lasso if emarry == 1 & female == 1 & bach_plus == `ed', lasiter(100) verbose(0) fdisplay(0)
local esel `r(selected)'

local msel : list ysel | esel

reg lwage exp exp2 `msel' if emarry == 1 & female == 1 & bach_plus == `ed', r
...
save gwg_lasso_wslope_`ed', replace
...
save gwg_lasso_slope_`ed', replace
