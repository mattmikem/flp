...
#delimit ;
global x_lasso = "educy uhrswork  race_* exp exp2
...
quietly lassoShooting lwage $x_lasso if emarry == 0 & nchild == 0 & female == 1, lasiter(100) verbose(0) fdisplay(0)
local ysel `r(selected)'
reg lwage `ysel' if emarry == 0 & nchild == 0 & female == 0, r
predict lwagehat_s if year == `y' & group == `j'
xx
quietly lassoShooting lwage $x_lasso if emarry == 0 & female == 1, lasiter(100) verbose(0) fdisplay(0)
local ysel `r(selected)'
